<script>
  ...
  // A custom Allmaps layer forces the lock on; the toggle is disabled so it
  // can't be turned off until the base is switched back to a tile layer.
  let lockForced = $derived(isCustomLayer("base") || isCustomLayer("overlay"));
</script>
...
<div class="flex max-w-full flex-wrap items-center gap-2">
  ...
  <div>
    <div class="group relative inline-flex w-15 shrink-0 rounded-full bg-gray-200 p-1 inset-ring inset-ring-gray-900/5 outline-offset-2 outline-gray-600 transition-colors duration-200 ease-in-out has-checked:bg-gray-600 has-focus-visible:outline-2 dark:bg-white/5 dark:inset-ring-white/10 dark:outline-gray-500 dark:has-checked:bg-gray-500 {lockForced ? 'opacity-50 cursor-not-allowed' : ''}">
      ...
      <input type="checkbox" name="setting" aria-label="Use setting" class="absolute inset-0 appearance-none focus:outline-hidden {lockForced ? 'cursor-not-allowed' : ''}" checked={mapState.lockLayers} disabled={lockForced} onclick={() => {
