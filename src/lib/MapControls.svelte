<script>    
    ...
    import { availableLayerCount } from './stores.js'; 

    let availableLayerCountValue;

    availableLayerCount.subscribe(v => {availableLayerCountValue = v});
  ...
  <section>
    ...
    <div class="control-tab-wrapper">
        <div class="control-tab" id="control-tab-map-controls" data-control-tab-value="map-controls" on:click={showHideControls}><Fa icon={faDraftingCompass} class="inline mr-2" />Controls</div>
        <div class="control-tab" data-control-tab-value="layer-controls" on:click={showHideControls}><Fa icon={faLayerGroup} class="inline mr-2" />Atlases <span class="bg-green-800 text-gray-200 text-s mx-0.5 px-1.5 py-0.5 rounded">{availableLayerCountValue}</span></div>
    </div>
