...
<div class="bg-white absolute rounded shadow bottom-3 left-2 right-2">
  ...
  <div class="p-3 rounded bg-gray-100">  <Fa icon={faPenToSquare} class="inline mr-2" /><span class="font-bold">
    ...
    <div class="py-2 px-3 border-t text-sm hover:bg-amber-50 text-gray-600 hover:text-gray-900 cursor-pointer {activeAnnotation===i ? 'bg-amber-100' : 'truncate'}" on:click={()=>{activeAnnotation=i; dispatch('moveMapToAnnotation',{'annotationIndex': i})} }>{a.data.body}</div>

    {/each}
    </div>

    <div class="p-3 bg-gray-100 flex">
      <button on:click="{()=>{dispatch('refreshAnnotations')}}" class="bg-blue-500 p-2 text-sm font-semibold rounded text-white">Reload annotations here</button>
