...
<div
  class="absolute top-5 right-5 z-30 overflow-hidden shadow-lg {expanded
    ? 'flex flex-col rounded-xl bg-white w-[85vw] max-w-xs max-h-[45vh] md:w-[20vw] md:min-w-64 md:max-w-md md:max-h-[max(20vh,11rem)]'
    : 'rounded-full'}"
>
  <div
    bind:this={toggleEl}
  ...
  {#if expanded}
    ...
    >
      ...
      <button
      onclick={collapse}
      class="text-xs text-white bg-gray-800 cursor-pointer rounded">No thanks</button>
